...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // $stmt = $conn->prepare("INSERT INTO orders (card_number, expiry_date, cvv, transaction_id) VALUES (?, ?, ?, ?) SELECT email FROM orders  WHERE email='$email'");
    ...
    // $conn = null;
    // Retrieve the customer name based on the customer email
$stmt = $conn->prepare("SELECT customer_name FROM users WHERE email = ?");
$stmt->bindParam(1, $email);
$stmt->execute();
$result = $stmt->fetch(PDO::FETCH_ASSOC);
...
$stmt = $conn->prepare("INSERT INTO orders (customer_name, email, card_number, expiry_date, cvv, transaction_id) 
                        VALUES (?, ?, ?, ?, ?, ?)");
$stmt->bindParam(1, $customerName);
$stmt->bindParam(2, $email);
$stmt->bindParam(3, $cardNumber);
$stmt->bindParam(4, $expiryDate);
$stmt->bindParam(5, $cvv);
$stmt->bindParam(6, $transactionId);
